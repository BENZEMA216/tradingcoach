...
5. **06-ai-coach.png** — "AI coach summarizes your problems and strengths." *(see open item §9)*
...
## 9. Open items before launch (not blocking the gallery)

- **AI Coach Chat** shows "AI Service Unavailable" (no LLM key on the server) and its summary
  renders in Chinese in EN mode. Decision pending: wire an LLM (OpenRouter) to enable Chat +
  an English summary, OR launch Insights-only and hide the Chat tab. See chat with maker.
