...
2026-05-22 进展：已先落地后端只读导入预检切片，`POST /api/v1/upload/trades/preview` 会识别券商格式、返回 broker、置信度、总行数、已成交行数、跳过行数、列名、错误和警告，不写数据库。

2026-05-23 进展：Landing 和 `/upload` 已接入导入预检 UI。用户选择 CSV 后会先看到券商识别、置信度、行数和可导入交易数；预检成功前不能开始分析。XLS/XLSX 不再被静默接受，会明确提示当前只支持 CSV。
