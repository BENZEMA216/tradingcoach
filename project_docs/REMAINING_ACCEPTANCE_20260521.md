...
| 类别 | 当前状态 | 说明 |
|------|----------|------|
| 自动化验证 | PASS | 2026-05-22 已复跑前端 typecheck/lint/unit/build、后端单元/数据完整性/集成路径；本轮新增 Chromium/Mobile Chrome 非视觉 e2e 均 72 expected / 0 unexpected |
| 局部浏览器 smoke | PASS | Dashboard、Statistics、Positions、Backtest 已渲染且无 `console.error` |
| 完整浏览器人工 QA | PASS | 2026-05-22 已覆盖 Landing/upload、Position detail、AI Coach、移动端、双语、评分筛选、多币种 |
...
| R-106 | `cd frontend && npm run test:unit` | 0 failures | PASS | 2026-05-22：9 passed |
| R-107 | `cd frontend && npm run build` | 构建成功 | PASS | 2026-05-22 已通过 |
...
| R-205 | Landing/upload | 文件选择、sample data、通知开关、恢复 banner、错误态 | 核心上传入口可用，失败态清楚 | PASS | Playwright：恢复 banner、文件选择、email、通知开关、执行按钮；无 console/page error |
...
| ID | 检查项 | 验证方法 | 通过标准 | 当前状态 | 备注 |
|----|--------|----------|----------|----------|------|
| R-501 | 没有 secrets | `rg -n "(sk-|api[_-]?key|secret|token|password|PRIVATE KEY|BEGIN RSA|AWS_ACCESS|OPENAI)" .` | 没有真实密钥；示例命中需解释 | PASS | 命中均为示例 env、配置读取、GitHub Actions secret 引用、依赖名或文档说明 |
