...
| 文件名 | 角色 | 功能 |
|--------|------|------|
| `__init__.py` | 模块入口 | 导出解析器类 |
| `csv_parser.py` | 中文CSV解析器 | 解析富途中文导出格式，字段映射 |
| `english_csv_parser.py` | 英文CSV解析器 | 解析富途英文导出格式 |
| `data_cleaner.py` | 数据清洗器 | 过滤无效订单、标准化字段、时区转换 |
| `incremental_importer.py` | 增量导入器 | 文件指纹去重、增量导入、历史记录 |
